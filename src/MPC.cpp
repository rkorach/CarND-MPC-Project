...
vector<double> MPC::Solve(Eigen::VectorXd state, Eigen::VectorXd coeffs) {
  ...
  //
  // 4 * 10 + 2 * 9
  // -> size(state) * N + size(actuators) * N-1 = 6 * 25 + 2 * 24 = 148
  size_t n_vars = 198;
  // DONE: Set the number of constraints
  // constraints are given by the MPC equations (6 of them, 4 for state + 5 for errors)
  ...
  return {solution.x[delta_start], solution.x[a_start],
          solution.x[x_start+0], solution.x[y_start+0],
